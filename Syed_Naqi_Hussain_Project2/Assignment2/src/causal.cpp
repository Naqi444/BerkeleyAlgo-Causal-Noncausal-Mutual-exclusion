...
void *RecvCast(void *sockfd)
{
  ...
  while(1)
  {
    ...
    //present vector values
    //cout << "Printing the present vector value" << endl;
    /*for (int d=0; d<nop; d++)
    {
      cout << vect[d];
    }
    ...
    else
    {
      ...
      int flag = CausalYorN(tmpstr);

      if (flag>0)   //If both the conditions of causality are satisfied
      {
        cout << "Multicast message received from process:" << "<" << flag <<">"<<" "<<"is delivered"<< endl;
        ...
        //After delivering a message above we go back to the queue to check if any of the messages can be delivered now
        if(!q.empty())
        {
          int res;
          do {
        ...
        pthread_mutex_unlock(&lock);    //I use a mutex here to make sure other messages get buffered in the while
      }
      else
      {
        if (!q.empty())
        {
          ...
          do{
            res = CheckQueue();
            if(res==1)   //This implies that a message from queue was delivered, so we go back to present message to check it it can be now delivered
            {
              if (flag==0)
              {
                int recheck = CausalYorN(tmpstr);
                if(recheck>0)
                {
                  flag = 1;
                  cout << "Multicast message received from process:" << "<" <<recheck<<">"<<" "<<"is delivered"<< endl;
                  vect[recheck-1]++;
          ...
          if(flag==0)
          {
            cout << "\n Received Message queued, as violating Causality 1" << endl;
            pthread_mutex_lock(&lock);
            q.push_back(tmpstr);
            pthread_mutex_unlock(&lock);
          }
        ...
        {
          cout << "\nReceived  Message queued , as violating Causality 2" << endl;
          pthread_mutex_lock(&lock);
          q.push_back(tmpstr);
          pthread_mutex_unlock(&lock);
        }
...
// Below is a thread function which all the processes run to act as server and wait for other processes to connect, using the accept() function
